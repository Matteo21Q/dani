type1error.NI.frontier <- function(p0.expected, p1.expected, NI.frontier, sample.size, 
  ...
  for (p0 in range.of.p0) {
    p1 <- ifelse(scale=="RD", p0+NI.frontier(p0), 
                                     ifelse(scale=="RR", p0*NI.frontier(p0),
                                            ifelse(scale=="OR", p0*NI.frontier(p0)/(1-p0+NI.frontier(p0)*p0), 
                                                   sin(NI.frontier(p0)+asin(sqrt(p0)))^2)))
