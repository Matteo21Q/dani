power.NIfrontier.binary <- function(p.control.expected, p.experim.target=NULL, NI.frontier, sig.level=0.025,
  ...
  p1.exp.null<-ifelse(summary.measure=="RD", p.control.expected+NI.frontier(p.control.expected), 
                      ifelse(summary.measure=="RR", p.control.expected*NI.frontier(p.control.expected),
                             ifelse(summary.measure=="OR", p.control.expected*NI.frontier(p.control.expected)/(1-p.control.expected+NI.frontier(p.control.expected)*p.control.expected), 
                                    sin(NI.frontier(p.control.expected)+asin(sqrt(p.control.expected)))^2)))
  ...
  if (isTRUE(print.out)) {
    cat("The estimated power with the NI.frontier provided and selected analysis method is ", power, "%, 95% Monte-carlo CI: [", power.low, "%, ", power.up, "%]")
  }
